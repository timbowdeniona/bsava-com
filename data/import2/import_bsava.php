...
function find_existing(string $class, string $field, string $value, int $parentId): ?DataObject\Concrete {
    ...
    $list = new $listClass();
    $list->setCondition("$field = ? AND parentId = ?", [$value, $parentId]);
    $list->setLimit(1);
    $objects = $list->load();
...
function import_books(string $filename, string $parentPath): void {
    ...
    foreach ($items as $item) {
        ...
        $existing = find_existing('Book', 'sku', $item['sku'] ?? $title, $folder->getId());
...
function import_ebooks(string $filename, string $parentPath): void {
    ...
    foreach ($items as $item) {
        ...
        $existing = find_existing('Ebook', 'sku', $item['sku'] ?? $title, $folder->getId());
...
function import_courses(string $filename, string $parentPath): void {
    ...
    foreach ($items as $item) {
        ...
        $existing = find_existing('Course', 'sku', $item['sku'] ?? $title, $folder->getId());
...
function import_events(string $filename, string $parentPath): void {
    ...
    foreach ($items as $item) {
        ...
        $existing = find_existing('Event', 'sku', $item['sku'] ?? $title, $folder->getId());
...
function import_membership_tiers(string $filename, string $parentPath): void {
    ...
    foreach ($items as $item) {
        ...
        $existing = find_existing('MembershipTier', 'sku', $item['sku'] ?? $name, $folder->getId());
        $obj = $existing ?? new DataObject\MembershipTier();

        $obj->setKey(Service::getValidKey($name, 'object'));
        $obj->setParent($folder);
        $obj->setPublished(true);
        $obj->setSku($item['sku'] ?? ''); $obj->setName($name);
...
/*
 * NOTE: Image fields (cardImage / coverImage) are not populated by this script.
 * Images must be uploaded to the Pimcore DAM first (Assets panel), then
 * either:
 *   a) Assigned manually in the admin UI by dragging from the Assets panel, or
 *   b) Set programmatically by adding to each import item:
 *
 *      $asset = \Pimcore\Model\Asset\Image::getByPath('/BSAVA/Images/my-image.jpg');
 *      if ($asset) {
 *          $obj->setCardImage($asset);   // for MembershipTier, Event, Course
 *          $obj->setCoverImage($asset);  // for Book, Ebook
 *      }
 *
 * Suggested DAM folder structure:
 *   /BSAVA/Images/Memberships/
 *   /BSAVA/Images/Events/
 *   /BSAVA/Images/Courses/
 *   /BSAVA/Images/Books/
 *   /BSAVA/Images/Ebooks/
 */
